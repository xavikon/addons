...
defaults = {auto_point = true,auto_gaze = true,auto_perm_gaze = false}
...
[3031]="Sylvan Slumber",[3032]="Crushing Gaze",[3358]="Blank Gaze",[3760]="Beguiling Gaze",[3898]="Chaotic Eye",[3916]="Jettatura",[4034]="Hypnic Lamp",[4036]="Bewitching Lantern",}
                ...
                if member and member.mob and (member.mob.id == v or member.mob.pet_index == index) or actor.id == v then
                    for ind, val in pairs(packet) do  
                        ...
                        elseif string.match(ind, 'Target %d+ Action %d+ Param') then --Turn on gazes than don't need the mob to face you to apply
                            if T{1694, 1695, 1713, 1716, 1762, 1950, 1978, 2155, 2814, 2817,4036}:contains(val) and angle.player < 90 then
                                return true
                            end
                        end
                    end
