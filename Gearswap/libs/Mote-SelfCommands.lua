...
function self_command(commandArgs)
    ...
    -- Allow jobs to override this code
    if not eventArgs.handled and extra_user_self_command then
        extra_user_self_command(commandArgs, eventArgs)
    end
...
function handle_autonuke(cmdParams)
	if #cmdParams == 0 then
		...
		autonuke = table.concat(cmdParams, ' ')
