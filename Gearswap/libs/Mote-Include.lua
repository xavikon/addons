...
function init_include()
	...
	state.RuneElement 		  = M('Ignis','Gelus','Flabra','Tellus','Sulpor','Unda','Lux','Tenebrae')
	state.ElementalMode 	  = M('Fire', 'Wind', 'Lightning', 'Light', 'Earth', 'Ice', 'Water', 'Dark')
	...
	windower.raw_register_event('prerender', function()
		...
		if (player ~= nil) and (player.status == 'Idle' or player.status == 'Engaged') and not midaction() and not moving and not (buffactive['Sneak'] or buffactive['Invisible']) and tickdelay > 1350 then
...
function handle_actions(spell, action)
    ...
    if not eventArgs.cancel then
        ...
        if _G['extra_user_'..action] then
            _G['extra_user_'..action](spell, action, spellMap, eventArgs)
        ...
        if not eventArgs.cancel and _G['extra_user_post_'..action] then
            _G['extra_user_post_'..action](spell, action, spellMap, eventArgs)
        end
...
function filtered_action(spell)
    ...
    if user_filtered_action then
        user_filtered_action(spell)
    end
	
    if extra_user_filtered_action then
        extra_user_filtered_action(spell)
    end

    -- Check jobs action filtering
    if job_filtered_action then
        job_filtered_action(spell)
    end
...
function get_melee_set()
	...
	if buffactive.sleep and sets.buff.Sleep then
...
function apply_defense(baseSet)
    if state.DefenseMode.current ~= 'None' then
        ...
        if extra_user_customize_defense_set then
...
function state_change(stateField, newValue, oldValue)
	...
	if stateField:contains('Auto') then
		tickdelay = 1350
