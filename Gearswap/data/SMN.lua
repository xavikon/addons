...
function job_pet_midcast(spell, action, spellMap, eventArgs)
	if spell.type == 'MagicalBloodPactRage' and sets.midcast.Pet.MagicalBloodPactRage[pet.name] then
		equip(sets.midcast.Pet.MagicalBloodPactRage[pet.name])
	elseif spell.type == 'PhysicalBloodPactRage' and sets.midcast.Pet.PhysicalBloodPactRage[pet.name] then
	...
	elseif spell.type == 'DebuffBloodPactWard' and sets.midcast.Pet.DebuffBloodPactWard[pet.name] then
...
function job_tick()
	if checkbuff() then return true end
	return false
end
...
function checkbuff()
	if state.AutoBuffMode.value and not moving and not areas.Cities:contains(world.area) then
		if not pet.isvalid then
			windower.send_command('input /ma "Titan" <me>')
			return true
		elseif not buffactive['Earthen Armor'] then
			windower.send_command('input /pet "Earthen Armor" <me>')
			return true 
		else
			return false
		end
