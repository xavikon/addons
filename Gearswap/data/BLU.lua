...
function job_setup()
    ...
    blue_magic_maps.MagicAccuracy = S{
        ...
        'Bad Breath', 'Blank Gaze','Blistering Roar','Blood Drain','Blood Saber','Cesspool','Chaotic Eye',
    ...
    blue_magic_maps.Stun = S{
        'Barbed Crescent','Battle Dance','Benthic Typhoon','Bilgestorm','Blitzstrahl',
...
function job_precast(spell, action, spellMap, eventArgs)
	if spell.action_type == 'Magic' then
		...
		if unbridled_spells:contains(spell.english) and not (buffactive['Unbridled Learning'] or buffactive['Unbridled Wisdom']) then
			eventArgs.cancel = true
			cancel_spell()
			add_to_chat(123,'Abort: Unbridled Learning not active.')
		
		elseif spellMap == 'Cure' or spellMap == 'Curaga' or (spell.skill == 'Blue Magic' and spellMap == 'Healing') then
...
function job_post_midcast(spell, action, spellMap, eventArgs)
    ...
    if spell.skill == 'Blue Magic' then
		...
		elseif spellMap:contains('Magical') then
			if state.MagicBurstMode.value ~= 'Off' and (buffactive['Burst Affinity'] or buffactive['Azure Lore']) then
	...
	if spell.english == 'Battery Charge' and sets.Self_Refresh then
